...
export default function EventArchive() {
  ...
  return (
    <div className="text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {archivedEvents.map((event, index) => {
            ...
            return (
              <motion.div
                key={event.id}
                className="bg-[#222222] border border-[#2A2A2A] rounded-lg p-6 hover:shadow-lg hover:shadow-black/20 hover:scale-[1.02] transition-all duration-300"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
      ...
      {/* Footer Stats */}
      <motion.div 
        className="max-w-6xl mx-auto mt-12 pt-8 border-t border-[#2A2A2A] px-6"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
